...
function GameCanvas({ lane, running, crashed, onCrash, tick }) {
  ...
  useEffect(() => {
    ...
    scene.add(new THREE.HemisphereLight('#8bc4ff', '#0b0718', 1.1));
    ...
    const baseCar = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.8, 4), carMaterial);
    ...
    player.position.set(LANES[laneRef.current], 0.52, 9);
    scene.add(player);
    ...
    const starPoints = new THREE.Points(starGeometry, new THREE.PointsMaterial({ color: '#8ec5ff', size: 0.4 }));
    ...
    const animate = () => {
      ...
      player.position.x += (LANES[currentLane] - player.position.x) * Math.min(delta * 12, 1);
      player.rotation.z = (LANES[currentLane] - player.position.x) * 0.12;
      player.position.y = isCrashed ? 0.72 : 0.52;
      carMaterial.emissive.set(isCrashed ? '#7f1330' : '#103b53');

      strips.forEach((strip) => {
        strip.position.z += delta * 48;
        if (strip.position.z > 16) strip.position.z = -340;
      });
      ...
      if (isRunning) {
        obstacles.forEach((obstacle, index) => {
          ...
          if (!isCrashed && !crashedThisFrame && hasCollision(currentLane, obstacle)) {
      ...
      neon.color.set(isCrashed ? '#ff3c76' : '#37f6ff');
...
function App() {
  ...
  useEffect(() => {
    const onKey = (event) => {
      if (event.key === 'ArrowLeft' || event.key.toLowerCase() === 'a') setLane((l) => shiftLane(l, -1));
      if (event.key === 'ArrowRight' || event.key.toLowerCase() === 'd') setLane((l) => shiftLane(l, 1));
      if (event.code === 'Space' && crashed) restart();
    };
